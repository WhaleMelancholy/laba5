...
int menu()
{
	...
	do
	{
		...
		{
			printf("Данного пункта меню не существует. Пожалуйста, попробуйте снова.\n");
			while (getchar() != '\n');
		}
...
Book** readFromFile(FILE* file, int &lengAdd)
{
	int size = 0;
	Book** readBooks = NULL;
	while (!feof(file))
...
int main()
{
	...
	do
	{
		...
		case 5: //Записать данные в файл
			...
			{
				FILE* file = fopen("C:\\Users\\Asus\\source\\repos\\gdgd\\data.txt", "w");
